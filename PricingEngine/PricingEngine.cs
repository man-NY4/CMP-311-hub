...
namespace PricingEngineApp
{
    public class PricingEngine
    {
        ...
        public decimal CalculateUnitPrice(int quantity, decimal retailPrice)
        {
            ...
            * 2. If quantity is greater than 10 but less than, or equal to 20,
            then 10% discount
            * 3. If quantity greater than 20, then 20% discount
            * 4. If it's a holiday and the total amount, after volume discount, is
            greater
            * than the holiday threshold, then apply the holiday discount
            percent
            * */
            ...
            if (retailPrice >= 1.00m)
            {
                ...
                if (_holiday & (quantity * (retailPrice * (1.00m -
                discountPercent)) > _holidayDiscountAmount))
                {
